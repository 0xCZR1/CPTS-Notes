...
# Common Credential Storage Locations and Hunting Methods
...
| wpa_supplicant | `/etc/wpa_supplicant/wpa_supplicant.conf` | `cat wpa_supplicant.conf` | `psk=` entries         |
...
| Mimipy         | Memory extraction                         | `python mimipy.py`        | Clear-text passwords   |
...
```shell-session
0xCZR@htb[/htb]$ python3.9 firefox_decrypt.py
